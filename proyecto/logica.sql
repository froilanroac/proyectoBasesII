...
IS
ID_DESTINO NUMBER;
REGISTROS_TABLA HIS_SERVICIOS%rowtype; 
CANTIDAD_PAQUETES NUMBER;
CURSOR SERVICIOS_DESTINO IS SELECT * FROM HIS_SERVICIOS WHERE FK_DESTINO = ID_DESTINO AND FK_SERVICIO <> 1 AND FK_SERVICIO <> 2 ORDER BY DBMS_RANDOM.RANDOM fetch first CANTIDAD_PAQUETES rows only;
CURSOR SERVICIOS_BASICOS IS SELECT * FROM HIS_SERVICIOS WHERE FK_DESTINO = ID_DESTINO  fetch first 2 rows only;
MAX_PAQUETE NUMBER;
NOMBRE_DESTINO VARCHAR(30);
FECHA_FIN DATE;
NUEVO_PRECIO_DIA NUMBER(20,2);
NUEVO_PRECIO_TO NUMBER(20,2);
BEGIN

    SELECT d.ID into ID_DESTINO FROM destinos d, his_servicios h where d.id = h.fk_destino ORDER BY DBMS_RANDOM.RANDOM fetch first 1 rows only;
    Select trunc(dbms_random.value(2,5)) num into CANTIDAD_PAQUETES From dual;
    SELECT MAX(ID) INTO MAX_PAQUETE FROM PAQUETES;
    MAX_PAQUETE:= MAX_PAQUETE + 1;
    SELECT D.DATOS.NOMBRE INTO NOMBRE_DESTINO FROM DESTINOS D WHERE D.ID = ID_DESTINO;
    ...
    while SERVICIOS_DESTINO%FOUND
        LOOP
        NUEVO_PRECIO_DIA:= REGISTROS_TABLA.FECHAS.PRECIO_DIA * 2;
        NUEVO_PRECIO_TO:= REGISTROS_TABLA.FECHAS.PRECIO_TO * 2;
        DBMS_OUTPUT.PUT_LINE('Destino ' || REGISTROS_TABLA.FK_DESTINO || ' SERVICIO ' || REGISTROS_TABLA.FK_SERVICIO );
        INSERT INTO HIS_PAQUETES VALUES (FECHA,FECHAS_TDA(FECHA,FECHA_FIN,NUEVO_PRECIO_DIA,NUEVO_PRECIO_TO),100,REGISTROS_TABLA.FK_SERVICIO,MAX_PAQUETE);
        FETCH SERVICIOS_DESTINO into REGISTROS_TABLA;
        END LOOP;
    CLOSE SERVICIOS_DESTINO;

    --insersion de crucero y alojamiento que siempre tienen que estar en un paquete
    OPEN SERVICIOS_BASICOS;
    FETCH SERVICIOS_BASICOS into REGISTROS_TABLA;
    while SERVICIOS_BASICOS%FOUND
        LOOP
        NUEVO_PRECIO_DIA:= REGISTROS_TABLA.FECHAS.PRECIO_DIA * 2;
        NUEVO_PRECIO_TO:= REGISTROS_TABLA.FECHAS.PRECIO_TO * 2;
        DBMS_OUTPUT.PUT_LINE('Destino ' || REGISTROS_TABLA.FK_DESTINO || ' SERVICIO ' || REGISTROS_TABLA.FK_SERVICIO );
